...
class BigEnemy extends Group{
        ...
        for (let object of this.parent.update_list) {
            ...
            if (object.obj_type == 'wall') {
                ...
                if(ret_norm > 1e-8) {
                    ...
                    var new_velocity = {
                        x: (this.velocity.x + 2 * ret_vector.x * dir_test) * discount_coef + ret_vector.x * 0.0005 / ret_norm,
                        y: (this.velocity.y + 2 * ret_vector.y * dir_test) * discount_coef + ret_vector.y * 0.0005 / ret_norm,
                        z: (this.velocity.z + 2 * ret_vector.z * dir_test) * discount_coef + ret_vector.z * 0.0005 / ret_norm
                    }
                    var energy = 0.5 * this.mass * (this.velocity.x * this.velocity.x + 
                                                    this.velocity.y * this.velocity.y +
                                                    this.velocity.z * this.velocity.z);
    ...
    apply_anti_force_from_position(position) {
        ...
        this.velocity.x += force_vector.x * 0.002 / f_norm;
        this.velocity.y += force_vector.y * 0.002 / f_norm;
        this.velocity.z += force_vector.z * 0.002 / f_norm;
    }
